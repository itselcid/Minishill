...
char *strjoin(const char *s1, const char *s2, const char *delimiter)
{
	size_t len1 = strlen(s1);
	size_t len2 = strlen(s2);
	size_t len_delim = strlen(delimiter);
	char *result = malloc(len1 + len2 + len_delim + 1);
	if (!result)
		return NULL;
	strcpy(result, s1);
...
int	ft_setenv(char *key, char *value, int overwrite)
{
    ...
    while (environ[i])
    {
        ...
        if (strncmp(environ[i], key, key_len) == 0 && overwrite)
        {
            if (overwrite == 1)
            {
			...
			{
				environ[i]=strjoin(environ[i],value,"");
			}
            return (0);
        }
...
int is_inenv(char **environ, const char *name) 
{
    int i = 0;
    char *key;
    char *val;
    ...
    while (data->cmd[i])
    {
        ...
        if (value)
        {
			if(name[strlen(name)-1] =='+')
			{
				...
				ft_setenv(name, value, 2);

			}
			else
            	ft_setenv(name, value, 1);
        }
        else if(!is_inenv(environ,name))
        {
			if(name[strlen(name)-1] =='+')
				name[strlen(name)-1] ='\0';
            ft_setenv(name, "+", 0);
